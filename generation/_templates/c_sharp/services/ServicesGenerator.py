...
class ServicesGenerator(BaseGenerator.BaseGenerator):
    ...
    def generate(self, lookup_template):
        ...
        for context in context_data.contexts:
            self.generate_service_interface(lookup_template, context)
            self.generate_service(lookup_template, context)
            self.generate_service_extension(lookup_template, context)
    ...
    def generate_service_extension(self, lookup_template, context_data):
        ...
        for file_path in glob.glob(f"{self.template_dir}/{lookup_template.template}/template_extension_registration.j2"):
            with open(file_path) as file:
                ...
                service = self.load_data(lookup_template)
                # for table in self.return_context_table(context_data):
                namespace = f"{self.project_data.base_namespace}.{self.template.namespace}.{context_data.context_name}"
                ...
                save_file_path = f"{self.project_data.basePath}/{self.template.base_path}/{context_data.context_name}/ApplicationServicesExtension.g.cs"
    ...
    def generate_service_interface(self, lookup_template, context_data):
        ...
        for file_path in glob.glob(f"{self.template_dir}/{lookup_template.template}/template_interface.j2"):
            with open(file_path) as file:
                ...
                j2_template = self.return_template(
                    loaded_template, lookup_template)
                services = self.load_data(lookup_template)
                for table in self.return_context_table(context_data):
                    namespace = f"{self.project_data.base_namespace}.{self.template.namespace}.{context_data.context_name}"
                    ...
                    save_file_path = f"{self.project_data.basePath}/{self.template.base_path_interface}/{context_data.context_name}/I{convert_to_case(table.table_name, self.domain.namingConvention)}Service.g.cs"
                    os.makedirs(os.path.dirname(
                        save_file_path), exist_ok=True)
                    with open(save_file_path, mode="w", encoding="utf-8") as message:
                        message.write(processed_template)

    def generate_service(self, lookup_template, context_data):
        print("Generating Services")
        for file_path in glob.glob(f"{self.template_dir}/{lookup_template.template}/template.j2"):
            with open(file_path) as file:
                ...
                for table in self.return_context_table(context_data):
                    ...
                    save_file_path = f"{self.project_data.basePath}/{self.template.base_path}/{context_data.context_name}/{convert_to_case(table.table_name, self.domain.namingConvention)}Service.g.cs"
                    os.makedirs(os.path.dirname(
                        save_file_path), exist_ok=True)
                    with open(save_file_path, mode="w", encoding="utf-8") as message:
                        message.write(processed_template)
